...
import type { ColumnDef, ColumnResizeMode, ExpandedState, Row } from "@tanstack/react-table"
...
type PaginationUpdater<TData> = (prev: { pageIndex: number; pageSize: number }) => {
  pageIndex: number
  pageSize: number
}
...
export function DataTable<TData extends ExportableData, TValue>({
  ...
  const getSelectedItems = useCallback(async () => {
    ...
    const idsToFetchRaw = selectedIdsArray.filter((id) => !idsInCurrentPage.includes(id))
  ...
  useEffect(() => {
    if (queryResult) {
      ...
      if (queryResult.isError) {
        setIsError(true)
        setError(
          queryResult.error instanceof Error ? queryResult.error : new Error("Unknown error"),
        )
      }
    }
  }, [queryResult])
  ...
  const getRowIdentifier = useCallback((row: TData) => String(row[idField]), [idField])

  const getSubRowsForRow = useCallback(
    (row: TData) => {
      const subRowsField = (subRowsConfig?.subRowsField ?? "subRows") as keyof TData
      const subRows = row[subRowsField]

      return Array.isArray(subRows) ? (subRows as TData[]) : undefined
    },
    [subRowsConfig?.subRowsField],
  )
  ...
  return (
    <div className="space-y-4">
      ...
      <div
        ...
        className="overflow-y-auto rounded-md table-container border"
                  ...
                  return (
                    ...
                    >
                      {row.getVisibleCells().map((cell, cellIndex) => (
                        <TableCell key={cell.id} className="px-4 py-2 truncate max-w-0 text-left">
                  ...
                  >
                    {row.getVisibleCells().map((cell, cellIndex) => (
                      <TableCell
                        ...
                        style={{
                          paddingLeft:
                            isSubRow && cellIndex === 0 ? `${indentSize + 16}px` : undefined,
                        }}
                      >
                        {flexRender(cell.column.columnDef.cell, cell.getContext())}
                      </TableCell>
